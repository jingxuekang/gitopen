...
const { setCache, getCache, CACHE_KEYS } = require('../../utils/cache.js')

const HOME_CACHE_KEY = CACHE_KEYS.HOME_DATA
...
const DEFAULT_CATEGORIES = [
  { id: 'tea', name: '茶叶', icon: '/images/icons/image-error.png' },
  { id: 'chenpi', name: '陈皮', icon: '/images/category/chenpi.png' },
  { id: 'teapot', name: '紫砂', icon: '/images/category/teapot.png' }
]
...
Page({
  data: {
    ...
    locating: false,
    locationName: '请选择位置',
    locationDetail: '定位后可查看附近配送信息',
    searchHint: '搜索白茶、陈皮、紫砂',
    memberLevel: 0,
    banners: [],
  ...
  async bootstrap(forceRefresh = false) {
    ...
    await Promise.all([
      ...
      this.loadHomeData(forceRefresh).catch(() => {})
    ])
  ...
  async loadCategoriesFromDB() {
    ...
    const categories = DEFAULT_CATEGORIES.map((def) => {
      ...
      return {
        ...
        icon: def.id === 'tea' ? def.icon : (remote.icon || def.icon)
      }
    })
  ...
  async loadHomeProductsFallback() {
    const db = wx.cloud.database()
    const queryProducts = async (orderField = 'sales', order = 'desc') => {
      try {
        const onlineRes = await db.collection('products')
          .where({ status: 1 })
          .orderBy(orderField, order)
          .limit(10)
          .get()
        const onlineList = Array.isArray(onlineRes.data) ? onlineRes.data : []
        if (onlineList.length) return onlineList
      } catch (err) {
        console.warn(`[首页兜底] status=1 查询 ${orderField} 失败`, err)
      }
      ...
      try {
        ...
        console.warn(`[首页兜底] 全量查询 ${orderField} 失败`, err)
        return []
      }
    }

    const [hotProducts, newProducts] = await Promise.all([
      queryProducts('sales', 'desc'),
      queryProducts('createTime', 'desc')
    ])
  ...
  normalizeProducts(list = []) {
    return (Array.isArray(list) ? list : []).map((item) => {
      const rawPrice = Number(item && item.price)
      const skuPrices = Array.isArray(item && item.skus)
        ? item.skus
          .map((sku) => Number(sku && sku.price))
          .filter((price) => Number.isFinite(price) && price > 0)
        : []
      const fallbackPrice = skuPrices.length ? Math.min(...skuPrices) : 0
      const resolvedPrice = Number.isFinite(rawPrice) && rawPrice > 0 ? rawPrice : fallbackPrice
      ...
      return {
        ...
        images,
        price: resolvedPrice,
        priceText: formatPrice(resolvedPrice || 0)
      }
    })
  },
  ...
  async loadHomeData(forceRefresh = false) {
    if (!forceRefresh) {
      ...
      if (cachedData && typeof cachedData === 'object') {
        const rankingSource = Array.isArray(cachedData.rankingSource) ? cachedData.rankingSource : []
        this.setData({
          ...cachedData,
          loading: false
        })
    ...
    // 无论是否配置了 banners，都尝试拉取首页业务数据（排行榜/新品/拼团）
    try {
      ...
      console.warn('[首页数据] getHomeData 失败，使用已有兜底数据', err)
      homeData = {}
    }

    if (!Array.isArray(homeData.hotProducts) || !homeData.hotProducts.length ||
      !Array.isArray(homeData.newProducts) || !homeData.newProducts.length) {
      const fallbackData = await this.loadHomeProductsFallback()
      if (!Array.isArray(homeData.hotProducts) || !homeData.hotProducts.length) {
        homeData.hotProducts = fallbackData.hotProducts || []
      }
      if (!Array.isArray(homeData.newProducts) || !homeData.newProducts.length) {
        homeData.newProducts = fallbackData.newProducts || []
      }
    }
  ...
  goToTrace(e) {
    ...
    wx.navigateTo({
      url: `/packageProduct/pages/trace/source/source?type=${category}`
    })
  },
  ...
  goToGroupList() {
    wx.navigateTo({
      url: '/packageOther/pages/group/list/list'
    })
  },
  ...
  goToGroup(e) {
    ...
    if (!id) return
    wx.navigateTo({
      url: `/packageOther/pages/group/detail/detail?id=${id}`
    })
  },
  ...
  getRankingPrice(item) {
    if (!item) return formatPrice(0)

    const directPrice = Number(item.price)
    if (Number.isFinite(directPrice) && directPrice > 0) {
      return formatPrice(directPrice)
    }

    const skuPrices = Array.isArray(item.skus)
      ? item.skus
        .map((sku) => Number(sku && sku.price))
        .filter((price) => Number.isFinite(price) && price > 0)
      : []
  ...
  restoreLocation() {
    const cached = wx.getStorageSync('userLocationDisplay')
    if (cached && cached.name) {
      this.setData({
        locationName: cached.name,
        locationDetail: cached.detail || ''
      })
    }
  },
  ...
  onTapLocation() {
    ...
    wx.chooseLocation({
      success: (res) => {
        const name = res.name || '已定位'
        const detail = res.address || `${Number(res.latitude || 0).toFixed(4)}, ${Number(res.longitude || 0).toFixed(4)}`
        this.setData({
          locationName: name,
          locationDetail: detail,
          locating: false
        })
        wx.setStorageSync('userLocationDisplay', { name, detail })
      },
      fail: () => {
        // chooseLocation 失败时兜底用经纬度定位
        wx.getLocation({
          type: 'gcj02',
          success: (res) => {
            const name = '当前位置'
            const detail = `${Number(res.latitude || 0).toFixed(4)}, ${Number(res.longitude || 0).toFixed(4)}`
            this.setData({
              locationName: name,
              locationDetail: detail,
              locating: false
