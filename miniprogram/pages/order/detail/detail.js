...
Page({
  ...
  onReview() {
    ...
    // 评价第一个商品
    const item = this.data.order.items[0]
    wx.navigateTo({
      url: `/pages/review/review?orderId=${this.data.orderId}&productId=${item.productId}&productName=${item.productName}&productImage=${item.productImage}`
    })
  }
})
